...
<div class="card">
  ...
  <div class="card-body">
      <form action="cherchertirage.do" method="get">
        <label>Mot Cle</label>
        <input type="text" name="motCle" value="${model.motCle}"/>
        <button type="submit" class="btn btn-primary">Chercher</button>
          <a  href="saisietirage.do" class="btn btn-primary">Ajouter</a>
			
    
      </form>     
      <table class="table table-striped">
        <tr>
          <th>ID</th><th>Nom Fichier</th><th>Date Envoyer</th><th>Date Traitement</th><th>Etat</th><th>Nom Enseignant</th><th>Suppression</th>
         </tr>
        <c:forEach items="${model.tirage}" var="c">  
           <tr>
              <td>${c.id }</td>
              <td>${c.nom_fichier }</td>
              <td>${c.date_env }</td>
              ...
              <td><a onclick="return confirm('Etes-vous sur ?')" href="supprimertirage.do?id=${c.id}">Supprimer</a></td>
